"""Source adapters for Karnataka KEA seat-matrix and allotment rows.

Adapters transform *external* rows (external column names + KEA tokens)
...
Seat-matrix input columns: ``Institute``, ``Course``, ``Category``,
``Quota``, ``TotalSeats`` (see contract).

Allotment input columns: ``Institute``, ``Course``, ``Category``,
``Quota``, ``Round``, ``OpeningRank``, ``ClosingRank``, ``SeatCount``
(Karnataka KEA machine-readable format).
"""
...
class KarnatakaSeatMatrixAdapter(SourceAdapter):
    """Transform Karnataka KEA seat-matrix rows into canonical ``SeatMatrix`` records."""
    ...
    def transform(
        ...
        for raw in raw_data:
            ...
            if not institute:
                skipped += 1
                continue
            category_id, pwd = normalize_karnataka_category(raw.get("Category", ""))
            records.append(
                {
                    "college_id": institute,
                    "college_name": institute,
                    "state": (raw.get("StateName") or "Karnataka").strip(),
                    "institute_type": "",
                    "quota_id": normalize_karnataka_quota(raw.get("Quota", "")),
    ...
    def validate_source(
        ...
        errors: list[str] = []
        if not raw_data:
            errors.append("Karnataka seat matrix source data is empty")
            return errors
...
class KarnatakaAllotmentsAdapter(SourceAdapter):
    """Transform Karnataka KEA allotment rows into canonical ``Allotment`` records.

    The adapter emits only rank/seat_count + cohort identifiers. Any
    ...
    def transform(
        ...
        for raw in raw_data:
            if not raw.get("Institute") and not raw.get("OpeningRank"):
                skipped += 1
                continue
            category_id, _pwd = normalize_karnataka_category(raw.get("Category", ""))
            records.append(
                {
    ...
    def validate_source(
        ...
        errors: list[str] = []
        if not raw_data:
            errors.append("Karnataka allotment source data is empty")
            return errors
        first_row = raw_data[0]
        errors.extend(
            f"Expected column not found: {column}"
...
__all__ = ["KarnatakaAllotmentsAdapter", "KarnatakaSeatMatrixAdapter"]
