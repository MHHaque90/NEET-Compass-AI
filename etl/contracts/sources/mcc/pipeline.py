"""MCC ingestion pipelines: contract -> validate -> transform -> persist.
...
exercisable in unit tests without PostgreSQL (see ADR-0011). The production
SQLAlchemy loader is wired in by the host application in Sprint 3.2.
"""
...
from etl.contracts.sources.mcc.parsers import parse_csv
...
def ingest_seat_matrix(
    ...
    *,
    ...
    """Ingest an MCC 2025 seat-matrix CSV through the full pipeline."""
    return _ingest(
        path,
        registry,
        loader,
...
def ingest_allotments(
    ...
    *,
    ...
    """Ingest an MCC 2025 allotment CSV through the full pipeline."""
    return _ingest(
        path,
        registry,
        loader,
