"""Source adapters for Uttar Pradesh NEET UG seat-matrix and allotment rows.

Adapters transform *external* rows (external column names + UP tokens)
...
(Uttar Pradesh machine-readable format).
"""
...
class UttarPradeshSeatMatrixAdapter(SourceAdapter):
    """Transform Uttar Pradesh seat-matrix rows into canonical ``SeatMatrix`` records."""
    ...
    def transform(
        ...
        for raw in raw_data:
            ...
            if not institute:
                skipped += 1
                continue
            category_id, pwd = normalize_up_category(raw.get("Category", ""))
            records.append(
                {
                    ...
                    "state": "Uttar Pradesh",
                    "institute_type": "",
                    "quota_id": normalize_up_quota(raw.get("Quota", "")),
    ...
    def validate_source(
        ...
        errors: list[str] = []
        if not raw_data:
            errors.append("Uttar Pradesh seat matrix source data is empty")
            return errors
...
class UttarPradeshAllotmentsAdapter(SourceAdapter):
    """Transform Uttar Pradesh allotment rows into canonical ``Allotment`` records.
    ...
    def transform(
        ...
        for raw in raw_data:
            if not raw.get("Institute") and not raw.get("OpeningRank"):
                skipped += 1
                continue
            category_id, _pwd = normalize_up_category(raw.get("Category", ""))
            records.append(
                {
    ...
    def validate_source(
        ...
        errors: list[str] = []
        if not raw_data:
            errors.append("Uttar Pradesh allotment source data is empty")
            return errors
        first_row = raw_data[0]
        errors.extend(
            f"Expected column not found: {column}"
...
__all__ = ["UttarPradeshAllotmentsAdapter", "UttarPradeshSeatMatrixAdapter"]
