"""Uttar Pradesh NEET UG provenance: deterministic file identity and SHA-256 checksums.

Two files from UP describing the same cohort must hash to the same checksum so
re-ingestion can be detected (idempotency) and audit trails can prove no rows
were silently dropped. The ``source_file_id`` is derived deterministically from
the checksum so the same bytes always produce the same identity, regardless of
when ingestion ran.
"""
