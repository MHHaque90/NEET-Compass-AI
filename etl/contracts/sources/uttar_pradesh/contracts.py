"""Uttar Pradesh NEET UG source contracts.
...
def _seat_matrix_validation_rules() -> tuple[ValidationRule, ...]:
    rules: list[ValidationRule] = [
        *_required("college_id", "course_id", "quota_id", "category_id", "total_seats"),
        ValidationRule("quota_id", "enum", {"values": ["ai", "so"]}),
        ValidationRule("category_id", "enum", {"values": ["gn", "bc", "ew", "sc", "st"]}),
...
def seat_matrix_2026_contract() -> SourceContract:
    """Contract for the Uttar Pradesh 2026 seat-matrix data.
...
def allotments_2026_contract() -> SourceContract:
    """Contract for the Uttar Pradesh 2026 allotment-result data.

    Declares the non-PII columns the Uttar Pradesh authority publishes in its
    machine-readable allotment file. Candidate PII columns are deliberately
    absent: the canonical Allotment record is built from rank + seat_count
    only.
    """
    return SourceContract(
        source_id="mcc_state_uttar_pradesh",
