...
from etl.contracts.sources.maharashtra.provenance import build_source_file_id, file_checksum
...
def test_full_ingest_seat_matrix_succeeds(seat_matrix_csv) -> None:
    ...
    expected_id = build_source_file_id(result.metadata.file_checksum, "mcc_state_maharashtra", "seat_matrix", 2026)
...
def test_three_runs_same_source_url_changed_bytes(tmp_path, seat_matrix_csv) -> None:
    ...
    run1 = ingest_seat_matrix(seat_matrix_csv, registry, loader, source_url="https://cetcell.mahacet.org/")
    assert run1.file_ingested is True
    assert run1.records_transformed == 8
    assert loader.count() == 8

    run2 = ingest_seat_matrix(seat_matrix_csv, registry, loader, source_url="https://cetcell.mahacet.org/")
    ...
    changed = tmp_path / "seatmatrix_mah_r1_2026_v2.csv"
    changed.write_bytes(seat_matrix_csv.read_bytes().replace(b"OP", b"GN", 1))

    run3 = ingest_seat_matrix(changed, registry, loader, source_url="https://cetcell.mahacet.org/")
    assert run3.file_ingested is True  # same source_url, new bytes
    assert run3.records_transformed == 8
    assert loader.count() == 8  # category unchanged (OP and GN both normalize to gn)

    assert run1.metadata is not None and run2.metadata is not None and run3.metadata is not None
    assert run1.metadata.source_url == run2.metadata.source_url == "https://cetcell.mahacet.org/"
    assert run3.metadata.source_url == "https://cetcell.mahacet.org/"
